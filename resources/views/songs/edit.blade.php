...
<div class="row justify-content-center my-5">
    <div class="col-lg-6">
        <div class="card shadow border-0 rounded-4 p-4">
            <h1 class="h3 fw-bold text-center text-primary mb-4">Editar Canción</h1>
            <form action="{{ route('songs.update', $song) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="mb-3">
                    <label for="name" class="form-label fw-semibold">Nombre</label>
                    <input type="text" name="name" id="name" value="{{ old('name', $song->name) }}" class="form-control form-control-lg" required>
                </div>
                <div class="mb-3">
                    <label for="artist" class="form-label fw-semibold">Artista</label>
                    <input type="text" name="artist" id="artist" value="{{ old('artist', $song->artist) }}" class="form-control form-control-lg" required>
                </div>
                <div class="mb-3">
                    <label class="form-label fw-semibold">Archivo MP3 (sube uno nuevo para reemplazar)</label>
                    <input type="file" name="file_upload" class="form-control">
                </div>
                <div class="mb-3">
                    <label class="form-label fw-semibold">o URL de la canción</label>
                    <input type="url" name="file_url" class="form-control" value="{{ old('file_url', (filter_var($song->file, FILTER_VALIDATE_URL) ? $song->file : '')) }}" placeholder="https://...">
                </div>
                <div class="mb-3">
                    ...
                    <select name="station_id" id="station_id" class="form-select form-select-lg">
                        ...
                        @foreach($stations as $station)
                            <option value="{{ $station->id }}" @if($song->station_id == $station->id) selected @endif>{{ $station->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="mb-4">
                    ...
                    <select name="playlist_id" id="playlist_id" class="form-select form-select-lg">
                        ...
                        @foreach($playlists as $playlist)
                            <option value="{{ $playlist->id }}" @if($song->playlist_id == $playlist->id) selected @endif>{{ $playlist->name }}</option>
                        @endforeach
                    </select>
                </div>

...
<div class="row justify-content-center my-5">
    <div class="col-lg-6">
        <div class="card shadow border-0 rounded-4 p-4">
            <h1 class="h3 fw-bold text-center text-primary mb-4">Editar Programación</h1>
            <form action="{{ route('schedules.update', $schedule) }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <select name="station_id" id="station_id" class="form-select form-select-lg" required>
                        ...
                        @foreach($stations as $station)
                            <option value="{{ $station->id }}" @if($schedule->station_id == $station->id) selected @endif>{{ $station->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="mb-3">
                    ...
                    <select name="playlist_id" id="playlist_id" class="form-select form-select-lg" required>
                        ...
                        @foreach($playlists as $playlist)
                            <option value="{{ $playlist->id }}" @if($schedule->playlist_id == $playlist->id) selected @endif>{{ $playlist->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="mb-3">
                    ...
                    <select name="day_of_week" id="day_of_week" class="form-select form-select-lg" required>
                        <option value="Todos" @if($schedule->day_of_week == 'Todos') selected @endif>Todos</option>
                ...
                <div class="mb-3">
                    ...
                    <input type="time" name="start_time" id="start_time" class="form-control form-control-lg" value="{{ $schedule->start_time }}" required>
                </div>
                <div class="mb-4">
                    <label for="end_time" class="form-label fw-semibold">Hora de fin</label>
                    <input type="time" name="end_time" id="end_time" class="form-control form-control-lg" value="{{ $schedule->end_time }}" required>
                </div>
